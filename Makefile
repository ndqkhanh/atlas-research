...
install: $(VENV) ## Install atlas-research + harness_core editable
...
run: install ## Start FastAPI locally on $(PORT)
	$(UVICORN) atlas_research.app:app --reload --port $(PORT)

tui: install ## Open the TUI against the running backend
	$(VENV_BIN)/atlas-research --url http://localhost:$(PORT)

tui-mock: install ## Open the TUI with the scripted demo (no backend)
	$(VENV_BIN)/atlas-research --mock
